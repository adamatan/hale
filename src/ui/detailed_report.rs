...
fn calculate_uptime_stats(history: &VecDeque<ProbeRound>, session_duration_secs: i64) -> UptimeStats {
...
fn determine_score(uptime_stats: &UptimeStats, incidents: &[SessionIncident]) -> SessionScore {
    ...
    let has_extended_outage = incidents
        .iter()
        .any(|i| i.duration_secs > 300.0);
